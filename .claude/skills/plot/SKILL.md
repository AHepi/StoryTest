...
| testing one plot choice (a reveal's timing, a turn's cause), or whether it does any work | Building step 8 below; `.claude/skills/hard-to-vary/SKILL.md`, section 5 of the hard-to-vary skill's `references/by-domain.md` | remove, move, swap, flip, two routes |
...
4. **The plot: a schedule of knowledge.** List the key facts of story *and* world. For each, write when the audience learns it, from whom, and with what emphasis. Draft this with a working teller in mind; step 5 may send you back. Choose its interest on purpose: *suspense* (they know what is at stake but not how it ends: show it early), *curiosity* (something happened that they have not been told: withhold a past fact) or *surprise* (they did not know there was a question: plant, hide, reveal, except a bare threat's arrival, `references/reveals-and-withholding.md`, section 2). Then mark every open question as a mystery or a wonder and check that the emphasis matches. Detail: `references/reveals-and-withholding.md`.
...
2. **A twist that fails.** Guessed: which fact came out too early? From nowhere: did each fact it depends on appear before it, clearly enough for a first-time audience to notice, and could this teller fairly have held it back? Either way: does the reveal change what someone decides or what the audience expects? If neither, it is information, not a turn. (`references/reveals-and-withholding.md`, sections 2, 3, 5 and 7.)
...
- **Surprise by default.** A hidden threat on its own usually buys seconds (it buys more when, for instance, its arrival breaks a guarantee early, as in *Psycho*, or starts new suspense); a shown one buys minutes. Truby's method hides the opponent's attacks; check each against the suspense it could have bought instead.
