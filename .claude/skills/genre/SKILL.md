...
- A **promise** is something the genre undertakes to deliver: the moments its audience comes for, and an answer to its **expected mystery**, the open question the genre leads the audience to expect the story to press (*who did it?*, *will they end up together?*; the name is the workshop's, owner question S8). Once the story presses it, it is a *mystery* (Gap principle 5), and the story must settle it. Breaking a promise reads as betrayal. (In the Gap theory the promise covers open questions, and emphasis inside the story sets it; the workshop stretches the word to the moments a genre owes, because dropping one reads the same way; owner question Q7.)
- A **guarantee** is a protection the audience assumes the genre gives, so part of it relaxes (the star survives, the child is safe). Broken early, it leaves the audience unsafe, not cheated (Anticipation principle 5); broken at the end, it leaves grief instead (the workshop's extension, owner question S5).
...
| testing whether a genre choice does any work | `.claude/skills/hard-to-vary/SKILL.md`, section 5 of the hard-to-vary skill's `references/by-domain.md` | remove, swap, flip, used lightly (Building step 9 says how) |

```mermaid
flowchart TD
  S["What is in front of you?"] --> Q{"Does a draft exist?"}
  Q -->|no, or adding a genre| B["Building, in this file"]
  Q -->|yes, or a pitch, and something feels off| D["Diagnosing, in this file"]
...
9. **Test each load-bearing genre choice** with `hard-to-vary`. For a craft note, use hard-to-vary lightly: its quick version (which carries the flip test) and the remove and swap tests in section 5 of the hard-to-vary skill's `references/by-domain.md`. Run its full procedure and report only when the writer asks whether a whole explanation holds up, such as their own account of why a draft fails. *Remove* a beat: which later scene loses its footing? *Swap* it for its stock version: if nothing changes, yours is doing no more than the default. *Flip* the guarantee you broke: if the story reads the same, the break did no work.
...
| "too bleak", "too flippant", in a story of one genre | a tone that breaks what the genre promises: the moments owed, or a protection the audience relaxed into (the workshop's reading, owner question S7) | the file's sections 2 and 4; the register of the lines themselves, `dialogue` |
