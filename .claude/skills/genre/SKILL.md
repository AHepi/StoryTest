---
name: genre
description: Chooses, builds, mixes and diagnoses genre - what a kind of story promises its audience, what it lets them assume, which protections they count on, its usual beats and the job each does, and how to twist it - built on what the owner's four theories say about genre and on Truby's The Anatomy of Genres, with twelve genre profiles. Use this whenever someone names a genre or a blend ("my horror western", "a cosy mystery", "a heist with a romance"), asks "what genre is this?", "what do fans expect?", "which beats am I missing?", "do they have to end up together?" or "can I kill the dog?", or says the story "doesn't feel like a thriller", "feels generic", "is too predictable", "is too dark for a comedy" or "will fans feel cheated?", even if they never say the word genre. Not for building the world (story-world), the order of events, reveals or suspense in general (plot), making readers care (character), wording lines (dialogue), or a critique of a whole work (story-critique leads).
---
...
Before a first full run in a conversation, read these passages in full, once: Implied World, Part 2 sections 1 and 2; Gap, principles 2, 5 and 7 and Part 2 sections 4 and 6; Anticipation, principles 1, 5 and 6 and Part 2 sections 1, 5 and 6; Bond, the ten levers and principle 4. The modules add detail from Truby's *The Anatomy of Genres* (the main book for this skill) and a few points from his *The Anatomy of Story*, in the workshop's own words. Where a book disagrees with a theory, the skill follows the theory and records the book's view as a rival.
...
- A **promise** is something the genre undertakes to deliver: the moments its audience comes for, and an answer to the question the audience expects the story to press. Once the story presses it, that question is a *mystery* (Gap principle 5), and the story must settle it. Breaking a promise reads as betrayal. (In the Gap theory the promise covers open questions, and emphasis inside the story sets it; the workshop stretches the word to the moments a genre owes, because dropping one reads the same way.)
...
- The genre's **question** is its outlook on how to live (Truby's *Mind-Action story view* and *thematic recipe*), kept here as a question the story answers by how it settles, never as a lesson.
...
This file holds the method. The modules hold detail; open one only when its row applies. Every genre file has the same thirteen sections, so "section 4" means the same thing in each.
...
| working in Memoir or Coming-of-Age | `references/genres/memoir-and-coming-of-age.md` | the examined life and the making of a self, truth as the teller's contract |
...
| testing whether a genre choice does any work | `.claude/skills/hard-to-vary/SKILL.md`, section 5 of its `references/by-domain.md` | remove, swap, flip |

```mermaid
flowchart TD
  S["What is in front of you?"] --> Q{"Does a draft exist?"}
  Q -->|no, or adding a genre| B["Building, in this file"]
  Q -->|yes, something feels off| D["Diagnosing, in this file"]
  Q -->|one small choice| K["The quick version, in this file"]
  B --> N["1 name the genre, blend and subgenre"]
  N -.->|unsure which, or more than one| HW["how-genres-work.md"]
  N --> F["2 open its file, or each file in a blend"]
  F --> FM
  F --> FC
  F --> FS
  F --> FO
  subgraph FM["mythic family"]
    MY["myth.md"]
    AC["action.md"]
    WE["western.md"]
  end
  subgraph FC["crime family"]
    CR["crime.md"]
    DT["detective-and-thriller.md"]
    GA["gangster.md"]
  end
  subgraph FS["speculative family"]
    FA["fantasy.md"]
    HO["horror.md"]
    SF["science-fiction.md"]
  end
  subgraph FO["outside the families"]
    CO["comedy.md"]
    LO["love-story.md"]
    ME["memoir-and-coming-of-age.md"]
  end
  F --> M{"3 more than one genre?"}
  M -->|yes: lead, clashes, sizes| HW
  M -->|no| R["4-5 four parts, beats as questions"]
  HW --> R
  R --> G["6 which guarantee to break"]
  G --> T["7 twist it"]
  T -.->|three routes| HW
  T --> C["8 care: character skill"]
  C --> H["9 hard-to-vary skill: test each choice"]
  D --> P["pin the symptom"]
  P --> W{"Which part is off?"}
...
- **Keep promises; guarantees can move.** Every question the story presses gets settled, though not always the way the audience hoped. One guarantee broken early and convincingly makes every later threat real (Anticipation principle 5). Broken late, the workshop adds, it gives the ending its weight instead.
...
- **Generic means riding on defaults.** The cure is one departure the story owns, not more beats done well.
...
Read the passages named under **Built on** before a first full run in a conversation.
...
9. **Test each load-bearing genre choice** with `hard-to-vary` (section 5 of its `references/by-domain.md`). *Remove* a beat: which later scene loses its footing? *Swap* it for its stock version: if nothing changes, yours is doing no more than the default. *Flip* the guarantee you broke: if the story reads the same, the break did no work.

### Diagnosing

1. **Pin down the symptom before explaining it.** To whom (a regular of the genre or a newcomer; regulars bring more baseline), where (the page, scene or minute it starts), compared with what (the genre's usual version, a work they name, an earlier draft). "It doesn't feel like a thriller" is not yet a symptom. "Two readers who know the genre well stopped worrying at the midpoint, where the husband is cleared and nothing threatens her again" is.
2. **Find which part of the genre is off.** First guesses, each to be tested:
...
| "generic", "a cliché", "seen it before" | the baseline: which defaults the draft rides on unchanged | step 4 below; `story-world` |
| "predictable", "I knew what was coming" | beats all in their stock order and form; a rhythm the audience has learned | the file's section 11; `references/how-genres-work.md`, section 9 |
...
3. **Build the rival diagnosis and tell the two apart.** Most symptoms have two plausible causes. "No tension" in a Thriller may be guarantees left standing, or care never built. Name a change that would fix one and not the other: break one guarantee on paper, or add a scene of the normal before the first threat, and see which the symptom answers to. Try the cheaper change first.
4. **For "generic", find the defaults, then sort them.** List what the draft takes from the genre's baseline unchanged: the setting, the hero, the opponent, beats in their stock form. *Swap* each for another stock version: the ones that survive every swap are the defaults the draft rides on. Then sort what survives. A world default gets one departure that would make the story its own, ideally one that embodies its question; hand its building to `story-world` (`.claude/skills/story-world/SKILL.md`). A stock hero or opponent goes to the file's section 5 and to `character`. A stock beat gets twisted (building step 7; the file's section 11). More beats will not cure it.
...
6. **Hand off** where the question crosses over: a world's departures and rules to `story-world`; the order of events, reveals, suspense and endings in general to `plot`; caring about the hero or the opponent to `character`; the talk a genre is known for (a Love Story's sparring, the questioning of suspects in Detective) to `dialogue`. For a critique of a whole work, `story-critique` leads if it is available, and this skill is consulted on genre.
...
2. What does that genre promise, and is every question the story presses settled by the end?
