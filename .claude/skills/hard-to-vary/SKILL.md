...
- **Flip the outcome.** Hold the stated starting conditions, non-outcome evidence and change-list position fixed. Had the opposite happened under those same conditions, could the explanation have covered it merely by changing its story after seeing the result? If yes, it explains nothing. If the opposite requires a named input, condition or part to differ, say which one and what would distinguish the cases. A conclusion that is *derived* (a theorem; a disjunction that follows from a count) covers every outcome because it is derived, not because it is loose: test it with the proofs tests in `by-domain.md` (remove a condition, weaken one, show a counter-case), not the flip. A derived conclusion is never put outside the test merely for being derived. Its status comes from those tests: *held* by a condition you name whose removal breaks it (where several hold it only together, say so); *idle*, and the result more general than stated, if none does.
- **Poke.** For each part, name one change to the world that should alter the outcome, and one that should not; the second must sit next to the disputed line, not remove the ability or the grounds wholesale. A pair that could be found for any part at all is not a pair. If no admissible separating pair exists, the distinction is only a label at this grain. If you simply cannot find or run such a pair yet, mark the part *unknown* and name the missing test. Watch the part as well as the outcome. Under each change ask two things: did the outcome move, and did the part move? A part that moves when you change only how it is read or reported is a measurement of the thing, and answers "what can we tell". The thing that produces the outcome stays put under that change. Keep hold of the same part through every change; sliding from "the reading" to "the thing read" swaps the claim under test.
- **Reverse.** Test the direction actually claimed. For a one-way production claim, poke the supposed cause and see whether the effect moves; separately poke the effect and check that the cause does not move merely because of that intervention. If feedback or a cycle is claimed, write both directions explicitly and test the intervention pattern the feedback claim predicts. Do not call an explanation backwards merely because changing the effect can later feed back into the cause.
...
- **Add a job.** Find something else the explanation already commits itself to if it is right. Does it survive unchanged? Did the new job rule out any rival version? If not, it added nothing. If you merely impose a new requirement, you changed the question; record that as a new question instead.
...
Six status marks, plus separate scope and dependency fields. The status marks (held, held if, two routes, loose, idle, unknown) say what the current test shows about a part's work. **Scope** says whether the part was in the test or the owner explicitly put that part outside it. **Dependency** says whether the part rests on another explanation not tested here; if so, name it. The three provenance words (fitted, built, asserted) say where a part came from; every part gets one provenance. The three job tags (given, fixed, added) say where a job came from and stay with the jobs. The design words in `by-domain.md` (held, free, inherited) and the how-I-know tags in `testing-against-cases.md` (seen, claimed, recalled, worked out) belong to their own files, and the two mappings across lists (*free* is loose and harmless, *inherited* is fitted; *recalled* is fitted) are mappings, said as such. Never use one field to answer another: *asserted* is not a status; *borrowed* is a dependency, not a status or provenance; owner-fixed is scope, not evidence that the part is held.
...
3. Flip the outcome under the same stated starting conditions. Could it have explained the opposite merely by changing the story? (Not for a derived conclusion: drop or weaken a condition instead.)
...
- **Stop** when every part that is in scope carries one of the six status marks: held (and by what; where two parts hold a job only together, say "held, jointly with" and name the other), held if (held only by a job that is itself in doubt, or only by a part marked loose, idle or unknown; name it and what would settle it), two routes (either of two parts does the job; name both), loose (and what could replace it), idle (removable), or unknown (and what test would settle it). Every part also gets **scope** (in test, or owner-fixed/outside the test), **dependency** (none named, or borrowed from another explanation; name it), and one provenance (fitted, built, asserted). An owner-fixed part is reported as *not assessed*, not as held or unknown, unless the owner later brings it into scope.
...
3. **Part by part:** for each in-scope part, one of six status marks: held (by which job or change), held if (which doubtful job, or which part marked loose, idle or unknown), two routes (which two parts), loose (what could replace it), idle (can be removed), or unknown (what would settle it). Beside that, record **scope** (in test, or owner-fixed/outside the test), **dependency** (none named, or borrowed from another explanation; name it), and **provenance** (fitted, built or asserted). For an owner-fixed part, write *status: not assessed*. A table is fine. Never add the marks up.
4. **Whole-explanation checks:** flip, reverse (test the direction actually claimed), answer hidden in the starting points, where the jobs came from, add a job, look inside, pairs that pull, check the patches (what was rescued before, and a gauge for every catch-all), what the change list leaves out, rivals, and where the parts came from (fitted, built or asserted, one for every part). (Remove, swap and poke are run part by part and reported in section 3.) Name every whole-explanation test and report **result**, **N/A**, or **not settled**. Use *N/A* only when the frozen question gives that test no target, and say why. Use *not settled* when the test is relevant but could not be run or lacks needed evidence; only then leave an affected in-scope part *unknown* with that test named. Never silently omit a test.
