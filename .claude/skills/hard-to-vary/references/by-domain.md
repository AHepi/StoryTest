...
Every decision gets one of three honest marks: **held** (name the requirement that forces it), **free** (any of several would do; say so, and pick for convenience), or **inherited** (there because it was there before). Trouble comes from free and inherited choices presented as forced. In the status marks used elsewhere in this skill, *free* maps to loose and harmless; *inherited* maps to fitted provenance. A free choice is not owner-fixed merely because it is free. When the design does not exist yet and you are building it, read `building.md`.
