...
- **Where each part came from.** Built (a test here forced it), fitted, or asserted (someone simply said so). For a builder, *fitted* includes "this is how such things are usually made, as far as I recall". Those are the first parts to check. Separately, record a **dependency** when a part rests on another explanation; name that explanation and its best rival. Borrowed is not a status or provenance.
