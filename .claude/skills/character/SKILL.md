---
name: character
description: Builds and diagnoses characters so the audience cannot stop caring what happens to them, using the owner's Bond theory (the ladder of care, the ten levers and where each breaks) for want, flaw, backstory, change, heroes, villains and casts, and tests each choice with hard-to-vary. Use this whenever someone is making a hero, villain, sidekick or cast, or says "I don't care about her", "he's flat", "she's unlikable", "is this out of character?", "my villain is just evil", "readers prefer the sidekick", "does he need an arc?", "what should she want?", "give him a backstory", "how do I make readers root for a killer?", "too many characters", or "nobody cried when she died". Not for what happens and when, suspense, reveals or endings (plot), how a character talks line by line (dialogue), the setting (story-world), a genre's usual hero (genre), or a critique of a whole work (story-critique leads).
---
...
| about to do a first full run in this conversation (a small case: see Built on) | `sources/bond-theory.md`, in full | the ladder, the seven principles, the ten levers and where each breaks, the limits of care |
...
| testing whether a trait, a ghost (a past event still driving them) or a scene is doing work | `.claude/skills/hard-to-vary/SKILL.md`, and section 5 of the hard-to-vary skill's `references/by-domain.md` | remove, swap, flip, and what holds a choice in place; how heavily to use it: Building, Step 8. Then come back here and reply as in Building, Step 9 |
...
```mermaid
flowchart TD
  S["What is in front of you?"] --> SC{"A small case?"}
  SC -->|"yes, and no row names it"| K["The quick version, in this file"]
  SC -->|"no, or a row names it"| Q{"Does the character exist yet?"}
  Q -->|"no, or a part is being added"| B["Building mode, in this file"]
  Q -->|"yes, and something feels wrong"| D["Diagnosing mode, in this file"]
  B -.->|"first full run"| T["sources/bond-theory.md (a small case: see Built on)"]
  D -.->|"first full run"| T
  B --> BC["building-a-character.md"]
  B --> CG["change-and-growth.md"]
  B --> CA["building-a-cast.md"]
  B --> OP["opponents.md"]
  D -->|"can't tell who they are, or why they act"| BC
  D -->|"no arc, unearned turn, out of character"| CG
  D -->|"too many, all alike, sidekick outshines hero"| CA
  D -->|"villain flat, just evil, or from nowhere"| OP
  BC --> H["hard-to-vary: remove, swap, flip"]
  CG --> H
  CA --> H
  OP --> H
  K --> H
  D -.->|"when things are learned, suspense, endings"| PL["plot skill"]
  D -.->|"how they talk"| DI["dialogue skill"]
  B -.->|"a genre's usual hero or villain"| GE["genre skill"]
  D -.->|"reads as the genre's stock type"| GE
```
...
**For an opponent the audience is meant to oppose**, read the rungs pointing the other way (the workshop's reading of principle 6, owner question S6, where the audience's wish for Joffrey's downfall is, in the theory's words, care pointing the other way). Recognise and Understand are built as for anyone. The upper rungs are turned: Side with turns round into wanting him stopped (this skill calls the turned rung *Side against*); Worry becomes fearing he will get what he wants, or get away with it; Miss becomes wanting his end, and the payoff when it comes (the cheer at Joffrey's death). Interiority still builds Understand, but ration it: lean on the ladder's other route, a clear motive, shown in scene (his case argued to someone, a reason the world gives him), and keep the audience out of his private company, or alignment tips the audience from Side against into Side with. How: `references/opponents.md`, section 8.
...
**Step 8 - Reply.** The last step in both modes: reply as in Building, Step 9, with the reply check that follows it.
...
Then reply as in Building, Step 9, with the reply check that follows it.
