---
name: dialogue
description: Writes, revises and diagnoses dialogue on page, stage and screen - what each line does to the listener, subtext, voice, exposition in talk, monologue, narration and inner talk, comic exchanges and silence - built on what the owner's four theories say about talk and on McKee's Dialogue, testing each line with hard-to-vary. Use this whenever someone is drafting or fixing a conversation, speech, voice-over or inner monologue, or says "the dialogue is stiff", "it's on the nose", "they all sound the same", "nobody talks like that", "too much exposition", "as you know, Bob", "the argument goes round in circles", "how do I show she's lying?", "the speech is too long", "the banter isn't funny" or "what's the subtext?", even if they never say the word dialogue. Not for when a fact comes out or who narrates (plot), making readers care about the speaker (character), a world's invented slang (story-world), a genre's promised beats (genre), or a critique of a whole work (story-critique leads).
---
...
This file holds the method: building dialogue, diagnosing it, and the quick version. The modules add depth. Open one only when its row applies. When a small case matches a specific row, open that row's module; the quick version is for a small case no row names. A short exchange that is the whole scene, and fails, is a case for Diagnosing, not the quick version.

| You are... | Open | To get |
|---|---|---|
| checking one line, or a short exchange in a scene that otherwise works | nothing else: "The quick version" below | four questions |
| about to do a first full run in this conversation (a small case: see Built on) | the owner-theory passages listed under "Built on", in full | what the frame says about talk |
...
| testing whether a line does any work | `.claude/skills/hard-to-vary/SKILL.md` (its quick version), and section 5 of the hard-to-vary skill's `references/by-domain.md` | remove, swap, flip, and what holds a choice in place; how heavily to use it: Building, Step 7 |
| finding the problem is when a fact comes out, or who tells the story | `.claude/skills/plot/references/reveals-and-withholding.md` (section 3 for a secret forced out, section 5 for exposition); `.claude/skills/plot/references/telling-and-perspective.md` | the schedule of knowledge, and what forces a secret out; the teller and fair play |
...
```mermaid
flowchart TD
  S["What is in front of you?"] --> Q{"Does the dialogue exist yet?"}
  Q -->|"no, or a line is being added"| B["Building, in this file"]
  Q -->|"yes, and something sounds wrong"| D["Diagnosing, in this file"]
  Q -->|"one line, or a short exchange in a scene that works"| K["The quick version, in this file"]
  B -.->|"first full run"| T["owner-theory passages on talk (a small case: see Built on)"]
  D -.->|"first full run"| T
  B -->|"wants, moves, subtext, exposition"| WD["what-dialogue-does.md"]
  B -->|"who is speaking"| V["voice.md"]
  B -->|"beats, turns, line shape"| LS["line-and-scene-design.md"]
  B -->|"which medium"| PS["prose-stage-screen.md"]
  D -->|"name the flaw family"| FF["flaws-and-fixes.md"]
  D -->|"they all sound the same"| V
  D -->|"it goes nowhere"| LS
  D -->|"stiff, on the nose, as you know"| WD
  WD --> H["hard-to-vary: swap the speaker, remove the line"]
  V --> H
  LS --> H
  FF --> H
  PS --> H
  K --> H
  D -.->|"when a fact comes out, who tells it"| PL["plot skill"]
  D -.->|"nobody cares who is talking"| CH["character skill"]
```
...
**What belongs in this skill.** One test for any addition: does it help decide what a character says or leaves unsaid, to whom, in what words, and whether each line does a job? When a fact should reach the audience goes to `plot`, and how much of the world to show to `story-world`, while how a fact is said in talk stays here; why the audience cares about the speaker to `character`; how a world's invented way of speaking works as a departure to `story-world`; what a genre's audience expects to hear, and whether the genre owes a kind of exchange, to `genre`, while how that talk is written stays here. When in doubt, leave it out and say why.
...
**Step 5 - Exposition as ammunition, surplus in passing.** List the facts the audience needs from this scene. Give each to a character with a reason to use it now: as a weapon, a probe, a question it opens, or an answer to someone who genuinely does not know; or, in prose, let the narrator tell it. Run the honesty test, McKee's, in the workshop's sharper form: does each speaker have a reason of their own to say this, to this listener, now, or is the line there only because the audience needs the fact? A hidden move is fine; a hidden author is not. Let world texture come in passing, taken for granted. When a fact should come out at all is `plot`'s (`.claude/skills/plot/references/reveals-and-withholding.md`, section 5; for a secret, section 3), and how much of the world to show is `story-world`'s (`.claude/skills/story-world/references/diagnosing-a-world.md`, section 6); how the fact is said is this step's. More: `references/what-dialogue-does.md`, section 7.
...
**Step 7 - Read it aloud, then test.** Read the scene aloud from each speaker's side and mark where you stumble, run out of breath or feel nothing. Then run `hard-to-vary` on the lines that matter. For a craft note, use hard-to-vary lightly: its quick version (which carries the flip test) and the remove and swap tests in section 5 of the hard-to-vary skill's `references/by-domain.md`. Run its full procedure and report only when the writer asks whether a whole explanation holds up, such as their own account of why a draft fails. The four tests below are those swaps, in dialogue's form:
...
**Step 1 - Pin the symptom.** Settle the medium first (and, in prose, the teller), even when another skill hands you the question: whether a line says too much or too little depends on it (`references/prose-stage-screen.md`, section 1). If it is unknown and the writer cannot be asked, read script format as screen and prose as prose, and say so. Then, before explaining anything, say to whom (all readers, some, an actor at a read-through), where (the line, exchange or speech) and compared with what (a scene in the same draft that works, an earlier draft, a named work). Freeze that question while you test. If the writer has given only a pitch or a snippet and cannot be asked, do not stall. Say in one line what you are assuming (for example, that "stiff" is every reader's complaint about the whole scene, compared with how these two would talk if each wanted something from the other), treat the draft's own elements as the symptom, and turn each missing fact into a check the writer can run (read the scene aloud with someone and have each of you mark the first line where you stop believing the speaker). This is hard-to-vary's rule: when facts are missing, do not guess; turn the missing fact into a test. If another skill has already pinned the symptom and hands you one question, take its pinning and go to the next step. Then read the complaint as a pointer, not a verdict:
...
| "too much exposition" | credibility, a fact at the wrong time, or too much of the world shown | `references/what-dialogue-does.md` section 7; when it comes out: `plot`; how much world: `.claude/skills/story-world/references/diagnosing-a-world.md`, section 6 |
...
**Step 5 - Fix the smallest thing, and test it.** Name the one want, move, line or word to change. When the whole scene fails, the smallest thing is the missing scene want (or wall) that Step 2 named, not a line. When the fix is a rewrite of the scene, rebuild it through Building Steps 1 to 7, keeping what the diagnosis said to protect. Test the fix with `hard-to-vary`, used lightly (Building, Step 7), on the line that forced it and on the lines it must leave alone. Check it does not knock out another job: cutting a fact a later turn needs, or stripping the wit that was carrying care.

**Step 6 - Hand off where the question crosses over.** When a fact should come out, what forces a secret out, and who is fair to hold it back: `plot` (`.claude/skills/plot/references/reveals-and-withholding.md`, sections 3 and 5; `.claude/skills/plot/references/telling-and-perspective.md`). Whether the audience cares about the speaker, or a line is "out of character": `character`. A world's invented way of speaking, as a departure that costs attention, or too much of the world on show: `story-world`. What a genre's audience expects to hear, and whether the genre owes a kind of exchange (a Love Story's sparring, the questioning of suspects in Detective): `genre` (the genre file's section 6); how that talk is written stays here. For a critique of a whole work, `story-critique` leads if it is available, and this skill is consulted on the dialogue.

### Reply

The last step in both modes. Reply in this order, in proportion to the case: what is wrong or missing, ranked, each with its reason and where it shows; what works and should be protected; which choices are free (they could be otherwise, and the writer should say so or replace them) and which are held; the fix for the top one or two, concrete to this draft; what you assumed or are unsure of; one next step. For a rewrite, show it, then say what changed and why. In dialogue, rank by the repair order (`references/flaws-and-fixes.md`, section 7), and for a rewrite also say where each fact of the original went.

<!-- shared: reply-check; carried by: character/SKILL.md, dialogue/SKILL.md, genre/SKILL.md, plot/SKILL.md, story-world/SKILL.md -->
**Before sending, check the notes themselves.** Do it in proportion to the case: a line for a quick note, each step for a full critique. For each fix, say what it gives up or puts at risk. Where a verdict depends on the writer's aim, give it both ways and ask. Where it depends on a fact nobody has told you (such as which of two drafts came first), never write as if you knew. Give the verdict both ways, ask, and use no words that assume an order; read every sentence that compares the drafts, since no list of words ("cuts", "adds", "becomes", "no longer") catches them all. Test each note: would it fit any story (a stock note: tie it to a line of the draft, or cut it), and would you have been as sure of the opposite? Let the result show: for each main note, say what it rests on, which is a line of the draft (or of the pitch) and what the point rests on (a theory principle; a book, marked fitted, built or asserted; or the workshop's reading, with its question number if it has one), or say that it is a guess. If the writer pushes back on a note, answer it in the reply. If the objection holds up (it "bears", in the `error-correction` skill's word), withdraw or change the note and say why. If the note assumed a fact the writer knows ("she's lying"), turn it into the question that fact opens (should the audience catch the lie now?). If the difference is the writer's aim, the aim decides. The reply is the whole record of it; only if the wrong note came from this skill's own text does it go further, as a correction (`error-correction` skill, "How much to do").
<!-- /shared -->

## The quick version

For one line, or a short exchange in a scene that otherwise works, four questions are enough.
...
4. Would fear, shame, manners or a tie keep this underneath? If so, does keeping it still cost less than telling it? If it does, keep it under the words; if silence now costs more, let it come out, as a turn. If nothing would keep it underneath, is saying it a move that costs the speaker something with this listener, or a report of what the audience already knows? Let them say it only when it is a costly move.

Then reply as in the Reply step.
...
- **Explaining an unpinned symptom without saying what you assumed.** "Stiff" for whom, in which line, compared with what? An unpinned complaint invites a loose explanation. When the writer cannot be asked, say what you assumed and turn the rest into checks (Diagnosing, Step 1).
