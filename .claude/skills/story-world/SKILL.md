---
name: story-world
description: Builds and diagnoses fictional worlds and settings, realist or invented - how the world differs from the real one, its rules and costs, societies, magic, technology, places and history, and how much of it to show - using the owner's Implied World theory, and tests each world choice with hard-to-vary. Use this whenever someone is worldbuilding, designing a magic system, a future, a city or a period setting, or says "my world feels generic", "it's just medieval Europe with dragons", "too much lore", "readers are lost", "it's an info-dump", "the setting is just a backdrop", "does this rule break my world?", "how much should I explain?", "the sequel made it feel smaller" or "how do I show the world on screen?", even if they never say the word world. Not for what happens and when (plot), making readers care about a person (character), wording lines of talk (dialogue), a genre's usual setting and promised beats (genre), or critiquing a whole work (story-critique leads).
---
...
| growing the world out of the story: the hero, the premise line, one bounded space, places that change as the hero changes | `references/world-from-story.md` | Truby's and Egri's methods, tied to breach, pressure and thematic fit, and the rival over indifference |
...
| checking one world choice: does it do any work? | Building, step 12, below; then `.claude/skills/hard-to-vary/SKILL.md`, section 5 of the hard-to-vary skill's `references/by-domain.md` | remove, swap, move, flip, poke; the check that keeps surplus |

When a small case matches a specific row, open that row's module; the quick version is for a small case no row names.

```mermaid
flowchart TD
  S["What is in front of you?"] --> Q{"Does a draft exist?"}
  Q -->|no, or adding a part| B["Building, in this file"]
  Q -->|yes, the world feels wrong| D["Diagnosing, in this file"]
  Q -->|a small case| K["The quick version, in this file"]
  Q -->|one world choice| H
  B -.->|starting from the hero or premise line| WS["world-from-story.md"]
  B --> B1["1-3 baseline, departures, price"]
  B1 --> B2["4-5 ripple; stakes and strain"]
  B2 --> B3["6-8 thematic fit, business, surplus"]
  B3 --> B4["9 reserve, surface, gaps"]
  B4 --> B5["10-11 medium, guide"]
  B5 --> H["12 hard-to-vary skill: test each choice"]
  B1 -.->|genre defaults| GE["genre skill"]
  B2 -.-> WM["world-materials.md"]
  B3 -.-> WM
  B4 -.->|scheduling mysteries| PL["plot skill"]
  B5 -.-> PF["prose-and-film.md"]
  D --> DW["diagnosing-a-world.md"]
  DW -->|a medium symptom| PF
  DW -.->|fear| PL
  DW -.->|care for a person| CH["character skill"]
  DW -.->|explaining in talk| DI["dialogue skill"]
  DW --> H
  K -.-> H
```
...
- **Every departure has a price in attention.** Spend on a few that matter. A world with nothing spent feels generic; a world with too much spent loses the audience.
...
- **Build the whole iceberg; show the tip.** The reserve is for the writer. The audience should feel its weight, not read it. (The rival, Truby's call to put as many of the world's rules on the page as possible, is in `references/diagnosing-a-world.md`, section 11.)
...
Read `sources/implied-world-theory.md` before a first full run in a conversation (for a small case or a handed-off question, see **Built on**).

### Building

Work in this order of logic, not necessarily the order of writing. A writer who starts from the hero or the premise line can find the places first with `references/world-from-story.md`; these steps then check the result.
...
12. **Test each load-bearing world choice** with `hard-to-vary`. For a craft note, use hard-to-vary lightly: its quick version (which carries the flip test) and the remove and swap tests in section 5 of the hard-to-vary skill's `references/by-domain.md`. Run its full procedure and report only when the writer asks whether a whole explanation holds up, such as their own account of why a draft fails. *Remove* a departure: if nothing breaks, ask whether it is surplus (it costs a clause, is taken for granted and belongs to this world); if so, keep it, and if not, it is idle: turn it back into baseline. *Swap* it for a near neighbour: if the story works as well, name what the two share, because that is what is really held. *Move* the whole story to the genre's default world or the audience's own, every departure gone at once: if the conflict survives unchanged, the world is not doing its job *(the Gap theory, principle 2: a breach with no world could happen anywhere; Egri builds the same test from one play)*. *Flip* a gap: fill it, and see whether the story shrinks.

### Diagnosing

1. **Pin down the symptom before explaining it.** To whom (which readers; genre regulars bring more baseline than readers new to the genre, so one page can be generic to one and overwhelming to another), where (the page, scene or minute it starts), compared with what (a work they name, an earlier draft, what they expected). "The world feels thin" is not yet a symptom. "The two readers who never read Fantasy stopped in the council chapter, where eleven noble houses are named" is. If the writer has given only a pitch or a snippet and cannot be asked, do not stall. Say in one line what you are assuming (for example, that the readers meant are new to the genre and the trouble starts where the invented terms first pile up), treat the draft's own elements as the symptom, and turn each missing fact into a check the writer can run (give the opening chapters to two readers new to the genre and have each mark the page where they lost track). This is hard-to-vary's rule: when facts are missing, do not guess; turn the missing fact into a test. If another skill has already pinned the symptom and hands you one question, take its pinning and go to the next step.
...
"Generic" about the setting is this skill's; about a whole story it goes to `genre`, about one person to `character`.
...
4. **Test the diagnosis with `hard-to-vary`,** lightly, as in Building step 12. Remove or swap the suspect departure on paper and say what would change for the reader. If the prediction is the same either way, the diagnosis is doing no work.
...
- **A world that exists only where the camera points.** Taking Truby's method literally (every place expresses the hero) produces it. Keep his method as a working order and keep the world's own business (`references/world-from-story.md`, section 11).
- **Giving every detail a meaning.** A world where every object is a symbol reads as a diagram. Keep a share of detail that means nothing (the rival: `references/world-materials.md`, section 9).
...
- **Explaining an unpinned symptom without saying what you assumed.** An account of "the world feels thin" that doesn't say for whom and where, or what it assumed in their place, cannot be wrong, so it cannot help.
